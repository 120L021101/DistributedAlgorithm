...
def report(num_nodes,num_msg,output_folder,algorithm):
    if algorithm == 'dolev':
        dolev_report(num_nodes,num_msg,output_folder)
    elif algorithm == 'bracha':
        bracha_report(num_nodes,num_msg,output_folder)
    elif algorithm == 'casual':
        casual_report(num_nodes,num_msg,output_folder)


def dolev_report(num_nodes,num_msg,output_folder):
...
def bracha_report(num_nodes,num_msg,output_folder):
    total_sent_msg_count = 0
    delivered_times = []
    all_node_delivered = True
    average_deliver_time_list = []
    ...
    # Loop through the range of file indices based on n
    for i in range(num_nodes):
        dolev_file_name = f"node-{i}.yml"
        dolev_file_path = os.path.join(output_folder, dolev_file_name)
        bracha_file_name = f"bracha-node-{i}.yml"
        bracha_file_path = os.path.join(output_folder, bracha_file_name)

        
        
        # Check if the file exists
        if os.path.exists(dolev_file_path) and os.path.exists(bracha_file_path):
            with open(dolev_file_path, 'r') as dolev_file, open(bracha_file_path,'r') as bracha_file:
                dolev_data = yaml.safe_load(dolev_file)
                bracha_data = yaml.safe_load(bracha_file)
                
                is_byzantine = bracha_data.get("is_byzantine", False)
                
                # Extract the records
                records = bracha_data.get("records", {})
                ...
                # Extract relevant data
                node_sent_msg_count = dolev_data.get("total_sent_msg_count", 0)
                node_delivered_msg = bracha_data.get("total_delivered_msg", 0)
                total_sent_msg_count += node_sent_msg_count
...
def casual_report(num_nodes,num_msg,output_folder):
    total_sent_msg_count = 0
    delivered_times = []
    all_node_delivered = True
    average_deliver_time_list = []
    print("IN CASUAL REPORT")
    ...
    for i in range(num_nodes):
        casual_file_name = f"causal-node-{i}.yml"
        casual_file_path = os.path.join(output_folder, casual_file_name)
        dolev_file_name = f"node-{i}.yml"
        dolev_file_path = os.path.join(output_folder, dolev_file_name)

        if os.path.exists(casual_file_path) and os.path.exists(dolev_file_path):
            with open(casual_file_path, 'r') as casual_file, open(dolev_file_path, 'r') as dolev_file:
                casual_data = yaml.safe_load(casual_file)
                dolev_data = yaml.safe_load(dolev_file)

                is_byzantine = casual_data.get("is_byzantine", False)

                records = casual_data.get("records", {})

                time_list  =[]
                for key, value in records.items():
                    avg_time = average(value.get("deliver_time"))
                    if avg_time > 0.1:
                        time_list.append(avg_time)
                    else:
                        time_list.append(0)
                    # time_list.append(average(value.get("deliver_time")))
                # print(time_list)

                # time_list = [records.get("time_spent", 0) for record in records.values() if record.get("time_spent") is not None]

                last_delivered_time = max(time_list,default=0)

                ave_time_list = [t for t in time_list if t > 0]
                average_deliver_time = sum(ave_time_list)/len(ave_time_list) if ave_time_list else 0
                delivered_times.append(last_delivered_time)
                ...
                node_sent_msg_count = dolev_data.get("total_sent_msg_count", 0)
                node_delivered_msg = casual_data.get("total_delivered_msg", 0)
                total_sent_msg_count += node_delivered_msg
                ...
                print(
                    ...
                    f"{node_delivered_msg:<18}"  # Recv Count
                    f"{node_sent_msg_count:<18}"  # Send Count
                    f"{last_delivered_time:<18.4f}"  # Max Time Spent
                    f"{average_deliver_time:<18.4f}"  # Average Time Spent
                )
    overal_avg_time = sum(average_deliver_time_list) / len(average_deliver_time_list)
    print("\n")
    print(f"All good = [{all_node_delivered}], Total msg sent: {total_sent_msg_count}, Last Delivered Time: {max(delivered_times)}, AVG Time: {overal_avg_time}")
...
@click.argument('topology_file', type=str, default='topologies/causal.yaml')
@click.option('--template_file', type=str, default='docker-compose.template.yml')
def compose_causal(num_nodes, broadcaster_num, broadcast_num_per_node, byzantine_num,\
                        vertex_degree, byzantine_behaviour, topology_file, template_file):
    prepare_compose_causal_file(num_nodes, broadcaster_num, broadcast_num_per_node, byzantine_num,\
                               vertex_degree, byzantine_behaviour, topology_file, template_file)
...
def prepare_compose_causal_file(num_nodes, \
    ...
    with open(template_file, 'r') as f:
        ...
        for i in range(num_nodes):
            ...
            # set False and 0 for dolev
            n['environment']['IS_BYZANTINE'] = False
            n['environment']['BYZANTINE_NUM'] = 0
            
            n['environment']['IS_BRB_BYZANTINE'] = flag_byzantine
            n['environment']['BRB_BYZANTINE_NUM'] = byzantine_num
            ...
            n['environment']['ALGORITHM'] = "causal"
...
def draw_topology2(topology_file: str, yaml_file: str="docker-compose.yml"):
    ...
    for node in G.nodes:
        ...
        for nodes in config['services'].values():
            ...
            if int(node) == int(nodes['environment']['PID']):
                is_broadcaster = nodes['environment']['IS_BROADCASTER']
                is_byzantine = nodes['environment']['IS_BRB_BYZANTINE']
